...
## [ARCHIVE] 04-design.md · v1 · 2026-07-19 11:26
...
# Design 鈥?omnipilot-web-guide MVP
...
### Edge Cases
- Happy: load extension 鈫?open side panel | Err: no active tab | Boundary: restricted chrome:// pages skip inject
...
2. Filter: hidden, tiny, decorative; dedupe; cap 40鈥?0; prefer viewport.
...
### Edge Cases
- Happy: form page 鈫?candidates | Err: empty body | Boundary: iframe (top-level only MVP)
...
### Implementation Logic
1. Options: API Base URL, model, API key 鈫?`chrome.storage.local` only.
2. Background: redact candidates 鈫?OpenAI-compatible chat 鈫?JSON schema validate (uid must exist).
...
### Edge Cases
- Happy: key set 鈫?features JSON | Err: 401/timeout 鈫?rules fallback | Boundary: empty howTo 鈫?template steps
...
4. Click list item 鈫?highlight; Tour next/skip/end.

### Edge Cases
- Happy: click feature 鈫?highlight | Err: uid detached 鈫?re-scan tip | Boundary: scrollable overflow
